...
final class Version20220303030333 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        foreach ($statement->fetchAllAssociative() as $legacyAccount) {
            $this->addSql('UPDATE na_user SET user_identifier = ?, banned = 0, reviewed = 1, needs_review = 0 WHERE id = ?', [
                Uuid::v4()->toBinary(),
                $legacyAccount['id'],
            ]);
        }
        ...
        foreach ($statement->fetchAllAssociative() as $userToken) {
            $this->addSql('INSERT INTO na_user (user_identifier, banned, hidden, reviewed, needs_review, created_at) VALUES (?, 0, 0, 1, 0, ?)', [
                Uuid::v4()->toBinary(),
        ...
        $this->addSql('UPDATE na_episode_chapter SET creator_id = LAST_INSERT_ID() WHERE creator_id IS NULL');
